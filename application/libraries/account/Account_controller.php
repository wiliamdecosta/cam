...
//            $table->setCriteria("b.account_status = 'OK'");
    ...
    function readLovTerminate() {

        $start = getVarClean('current','int',0);
        $limit = getVarClean('rowCount','int',5);

        $sort = getVarClean('sort','str','termination_reason_id');
        $dir  = getVarClean('dir','str','asc');
        $searchPhrase = getVarClean('searchPhrase', 'str', '');
        ...
        try {
            ...
            $ci = & get_instance();
            $ci->load->model('lov/terminate_reason');
            //$table = $ci->product;
            $table = new terminate_reason(); //kalau ada param
            ...
            if(!empty($searchPhrase)) {
                $table->setCriteria("(upper(termination_reason_id) ".$table->likeOperator." upper('%".$searchPhrase."%')
    ...
    function readLovBusinessShare() {

        $start = getVarClean('current','int',0);
        $limit = getVarClean('rowCount','int',5);

        $sort = getVarClean('sort','str','p_business_area_id');
        $dir  = getVarClean('dir','str','asc');
        $searchPhrase = getVarClean('searchPhrase', 'str', '');
